...
using TMPro;//인터페이스 기능(여러개 상속 가능)
...
public class Slot : MonoBehaviour, IPointerEnterHandler, IPointerExitHandler, IPointerClickHandler, IBeginDragHandler, IDragHandler, IEndDragHandler, IDropHandler
{
    ...
    // 필요한 컴포넌트
    [SerializeField] private TextMeshProUGUI text_Count; //아이템 개수 텍스트
    [SerializeField] private GameObject go_CountImage; // 아이템 개수 카운트 이미지
    ...
    public void AddItem(Item _item, int _count = 1) // 매개변수 : 얻는 아이템, 얻는 갯수(기본값 = 1)
    {
        ...
        itemImage.sprite = AssetManager.Instance.atlas.GetSprite(_item.sprite_name); // itemImage의 sprite를 item.itemImage(획득 아이템의 이미지)로 교체

        if (item.item_type != "장비") // 아이템의 타입이 장비아이템이 아닐 경우
        {
    ...
    // 슬롯 초기화.
    private void ClearSlot() //아이템 카운트가 0이거나 0보다 작을 경우(아이템을 다 썼을 경우) 실행
    {
    ...
    public void OnPointerClick(PointerEventData eventData)
    {
        ...
        //        if(item.itemType == Item.ItemType.Used) // item의 타입이 Used일 경우
    ...
    {
        if(item != null)
            inven.slotToolTip.ShowToolTip(item, transform.position);
    }

    // 슬롯에서 빠져나갈 때 발동.
    public void OnPointerExit(PointerEventData eventData)
    {
        inven.slotToolTip.HideToolTip();
    }
}